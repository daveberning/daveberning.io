...
/* Page Content
--------------------------------- */
const [ { data: index }, { data: allTestimonials } ] = await Promise.all([
  useAsyncData('testimonials-index', () => queryCollection('content').path('/testimonials').first()),
  useAsyncData('testimonials', () => queryCollection('testimonials').all()),
])
